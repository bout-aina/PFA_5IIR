...
import os
import uuid
from flask import Flask,session, flash, request, redirect, url_for, render_template, send_file, abort
...
def liste_cv():
    files = _get_files()
    return render_template("liste_cv.html", files=files)


@app.route('/logout',methods=["POST"])
def logout():
    if request.method == 'POST':
        session["if_logged"] = None
...
def admin():
    if request.method == 'POST':
        ...
        if (user == "admin") and (passwrd == "Admin2021"):
            session["username"] = True
            session["if_logged"] = True
            return redirect(url_for("liste_cv"))
